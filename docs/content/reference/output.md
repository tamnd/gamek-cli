...
describes what users get. Pick a format with `-o`, or let gamek choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
gamek <command> -o table   # aligned columns for reading
gamek <command> -o jsonl   # one JSON object per line, for piping
gamek <command> -o json    # a single JSON array
gamek <command> -o csv     # spreadsheet friendly
gamek <command> -o tsv     # tab-separated
gamek <command> -o url     # just the URL column
gamek <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
gamek <command> --fields id,title,url
```
...
```bash
gamek <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
gamek <command>            # a table, because this is a terminal
gamek <command> | wc -l    # JSONL, because this is a pipe
```
